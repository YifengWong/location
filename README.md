...
### 3.1 系统整体设计
系统分为三个主要端（模块）：服务端、客户端、算法端。

1. 服务端主要基于JavaWeb开发后台，提供RESTful接口给客户端；同时提供C++配套模块接口，给算法端进行调用。
2. 客户端基于Android操作系统开发APP，获取用户捕捉的图片信息、传感器数据并回传给服务端，服务端在完成定位后将返回结果。
3. 算法端基于深度学习的图像匹配算法。
...
1. 使用Spring框架，提供RESTful接口，这些接口的设计应当易于拓展、维护。
2. 服务端暂时不提供持久化信息的服务，因此对请求都是无状态的处理。
3. 提供与C++互相调用的解决方案，其中组件应当是可替换、易于维护的。

#### 3.2.2 C++接口内容

1. 提供C++的接口，这个模块应当易于使用，在算法端引入模块即可直接调用。
2. 完全解耦封装，与Java Web调用的解决方案应对算法端透明。
...
2. Java Web应用接收图片并整理顺序保证时序无错乱后，将图片帧存传递给C++入口模块。
3. C++部分代码提供封装好的接口，可以读取图片等信息，并提交给算法进行处理。
4. 算法处理完毕后返回结果，通过消息传递解决方案机制回传给Java Web。
5. Java Web可以定时查询所需结果。

#### 4.1.3 Java Web应用设计
##### 4.1.3.1 主要包 location.message 内容
![](https://github.com/YifengWong/location/blob/master/docs/pics/java-message-package-classes.png)

##### 4.1.3.2 实体类 Message
1. 作为与C++沟通的主要消息实体，可以用于保持一个具体的消息。包括请求用户ID、请求类型、请求文件。
2. 一个具体的消息分为多种，有图片类型，结果类型，可以与C++算法部分进行完备的交流。
3. 消息还可以保持前端发送过来的传感器数据，作为参数存放。
4. 有序列化与反序列化方法，作为内存对象写入与C++一致。

##### 4.1.3.3 AbstractMsgService以及子类不同实现 RedisMsgService 和 SocketMsgService
...
##### 4.1.4.3 序列化
将Message类的对象，通过内存拷贝的方式进行传输。这样可以提高效率而且易于维护，Java与C++统一此
传输格式。
...
### 5.1 红字1
AbstractMsgService具体完成了两个实现：SocketMsgService以及RedisMsgService。前者以Socket方式进行Java与C++接口之间的通信，后者通过
Redis内存缓存服务进行通信。在一开始的实现中，首先尝试使用了Socket方式。因为此方式对第三方组件没有依赖，是系统级别的的通信过程。完成后进行
测试等流程，包括对消息的序列化、反序列化后的延迟统计发现，该方式的延迟较高，甚至令人难以接受，每消息达到了近100毫秒的延迟。随后马上进行了
对序列化过程的优化方案，如循环算法的优化、传输时写入缓冲区调用内存拷贝接口，但结果的延迟降低并没有很好的效果。确实，对于TCP/IP协议栈的优化
还有许多需要学习的地方。为了降低开发成本，我选择转入使用内存共享缓存的方案解决该问题，于是选择了Redis的接口。由于该部分的耦合结构在原先已有
考虑设计，只需直接继承AbstractMsgService类实现Redis方案。在开发过程中发现，Redis方案的开发效率得到了提高，其维护成本、代码易读性都较
Socket方案有了很大的改善。由于是在应用级别的实现，最终系统的可移植性也会得到保证。完成后的测试也给了我们更大的信心，几乎不需要什么改动，
每一个消息的通信延迟直接降低到了5ms！
